...
RUN set -x \
  && wget --auth-no-challenge --user=XDelphiGrl --password=${{secrets.XDELPHIGRL_JENKINS}} -O ${LIQUIBASE_VERSION}.tar.gz "https://jenkins.datical.net/job/liquibase-pro/job/DAT-6750/75/artifact/liquibase/liquibase-dist/target/${LIQUIBASE_VERSION}/liquibase-${LIQUIBASE_VERSION}.tar.gz" \
